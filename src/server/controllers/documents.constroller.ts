...
import { CreateDocumentDto, DocumentByIdDto, DocumentsListDto, UpdateDocumentDto } from '../dtos';
...
export class DocumentsController {
    ...
    public async getById(payload: DocumentByIdDto): Promise<DocumentType> {
      const document = await prisma.document.findFirst({ where: { id: payload.id } });
    ...
    public async update({ id, ...payload }: UpdateDocumentDto): Promise<DocumentType> {
